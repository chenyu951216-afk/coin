...
class ScanConfig:
    ...
    max_symbols: int = 0  # 0 = all Bitget tradable contracts after public prefilter
...
def _closed_window(timeframe: str, bars: int) -> tuple[str, str]:
    ...
    # End at the opening timestamp of the most recently completed bar.
...
def derive_strategy_levels(df: pd.DataFrame, signal: StrategySignal, entry: float | None = None) -> dict[str, float]:
    """Mirror the current backtest's initial ATR + confirmed-structure exits.

    This is intentionally a read-only mirror. It does not tune or modify the
    strategy rules; it exposes the same current strategy stop/target geometry
    for scanner candidates and live execution handoff.
    """
    if df.empty:
    ...
    return {"entry": entry_price, "stop_loss": stop, "take_profit": target, "atr": atr}
...
def scan_market(
    *,
    ...
    # Do not spend six history requests on a symbol CoinGlass cannot resolve.
    ...
    for i, candidate in enumerate(candidates, 1):
        ...
        try:
            ...
            frame = frame.dropna(subset=["atr14"])
